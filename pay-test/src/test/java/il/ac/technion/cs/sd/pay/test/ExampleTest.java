...
public class ExampleTest {
    ...
    private static PayBookReader setupAndGetReader(String fileName) throws Exception {
        String fileContents =
                new Scanner(new File(ExampleTest.class.getResource(fileName).getFile())).useDelimiter("\\Z").next();
        Injector injector = Guice.createInjector(new PayBookModule(), new TestingSecureDatabaseModule());
